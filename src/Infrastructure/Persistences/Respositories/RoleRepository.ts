...
export default class RoleRepository extends BaseRepository<Role> implements IRoleRepository{
    ...
    async getRoleById(roleId: number | mongoose.Types.ObjectId, queryData: any): Promise<RoleWithBase | null> {
        try {
    ...
    async createRole(createRoleData: any): Promise<RoleWithBase> {
        try {
    ...
    async getRoleByName(roleName: string, queryData: any): Promise<RoleWithBase | null> {
        try {
            const query = {
                name: roleName,
                isActive: queryData.isActive,
            ...
            const role: RoleWithBase[] = await this.findDocuments(query, null, {});
